...
public class Main{
    ...
    public static void main(String[] args) throws IOException {
        ...
        BufferedImage ImagemResultado = new
...
//        recolorirUmaThread(ImagemOriginal, ImagemResultado);
    ...
    public static void recolorirImagem(BufferedImage ImagemOriginal, BufferedImage
            ImagemResultado, int leftCorner, int topCorner,
                                       int width, int height) {
        for(int x = leftCorner ; x < leftCorner + width && x <
                ImagemOriginal.getWidth() ; x++) {
            for(int y = topCorner ; y < topCorner + height && y <
                    ImagemOriginal.getHeight() ; y++) {
                recolorirPixel(ImagemOriginal, ImagemResultado, x , y);
            }
        }
    }

    public static void recolorirPixel(BufferedImage ImagemOriginal, BufferedImage
            ImagemResultado, int x, int y) {
        ...
        //se o pixel em quest o for um tom de cinza, vamos aumentar o n vel de� � vermelho em 10; o de verde diminuir 80,
        // azul dimiuir 20
        if(ehNivelDeCinza(red, green, blue)) {
        //para n o exceder o valor m ximo (255) pegamos o min� �
            newRed = Math.min(10, red + 100);
